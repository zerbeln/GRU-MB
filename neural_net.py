...
class NeuralNetwork:
    ...
    def __init__(self, parameters, mean=0, std=1):
        ...
        self.bias = 1.0
        self.n_tsteps = parameters["t_steps"]

        # Feedforward Neural Network
        self.ffn_inputs = int(self.n_inputs + self.mem_block_size)
        ...
        # Read Gate
        self.num_rgw = (self.mem_block_size+1)*self.mem_block_size
        ...
        # Write Gate
        self.num_wgw = (self.mem_block_size+1)*self.mem_block_size
        self.wgate_inputs = np.mat(np.zeros(self.mem_block_size+1))
    ...
    def get_weights(self, weights):  # Get weights from CCEA population
        ...
        self.rgate_weights = np.reshape(self.rgate_weights, [self.mem_block_size+1, self.mem_block_size])

        # Write Gate Weights
        for w in range(self.num_wgw):
            self.wgate_weights[0, w] = weights["wgate_weights"][w]
        self.wgate_weights = np.reshape(self.wgate_weights, [self.mem_block_size+1, self.mem_block_size])
    ...
    def run_neural_network(self, tstep, state_vec):
        """
