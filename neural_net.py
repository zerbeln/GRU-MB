...
class NeuralNetwork:
    ...
    def reset_nn(self):  # Clear current network
        ...
        self.out_bias_weights = np.mat(np.zeros(self.n_outputs))  # 1x1
    ...
    def get_weights(self, weights):  # Get weights from CCEA population
        ...
        self.wgate_weights["K"] = np.reshape(np.mat(weights["k_wgate"]), [self.mem_block_size, 1])  # nx1
        self.wgate_weights["R"] = np.reshape(np.mat(weights["r_wgate"]), [self.mem_block_size, 1])  # nx1
